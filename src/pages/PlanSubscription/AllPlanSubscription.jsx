...
const AllPlanSubscription = () => {
    ...
    const hasPermissionAdd = useCheckPermissionByPage("Plan Subscription", "insert");
    const hasPermissionDelete = useCheckPermissionByPage("Plan Subscription", "delete");
    const Data = [
        {
            id: 1,
            name: "Basic",
    ...
    const columns = [
        ...
        { key: "status", label: "Status", renderCell: (key, row) => <InputSwitch checked={row?.status} onChange={() => handleStatusChange(row.id)} /> },
        {
            key: "action", label: "Action", renderCell: (key, row) => <div className="flex items-center space-x-2.5">
                <span className="icon-edit text-[18px] lg:text-[20px] xl:text-[24px] text-g1 cursor-pointer" onClick={() => navigate(`./edit/${row.id}`)}></span>
